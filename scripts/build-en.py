...
TEXT = {
    ...
    '网页操作繁琐': 'Too many steps in the browser',
    '看项目、回复问题、审查改进请求和下载文件，常要在浏览器多个页面之间来回切换；下载进度和后续操作也不集中。': 'Browsing projects, replying to issues, reviewing proposed changes, and downloading files often means switching between pages. Download progress and next steps are scattered too.',
    '一个窗口集中处理，下载进度清楚可见': 'Handle it in one window, with clear download progress',
    '核心特性': 'Features',
