...
TEXT = {
    ...
    '同类对比': 'Comparison',
    '开始使用': 'Get started',
    '开源 · GPL-3.0 · Windows 桌面客户端 v1.0.2': 'Open source · GPL-3.0 · Windows desktop app v1.0.2',
    '让 GitHub': 'Make GitHub',
    '简单到每个人都会用': 'simple for everyone',
    '不用学命令行，也不用安装 Git。选择项目文件夹、写一句更新说明，就能发布源码；还可以单独发布带介绍和下载文件的新版本。': 'No command line or Git installation required. Choose a project folder, write a short update, and publish your code. When you are ready, create a separate release with a description and downloadable files.',
    '下载与运行': 'Download',
    '下载': 'Download',
    ...
    '选择任意文件夹即可识别为项目；独立 Worker 扫描新增、修改、删除、重命名的文件，遵守 .gitignore、避开依赖目录，变化时防抖复检。基于 isomorphic-git，无需安装 Git。': 'Choose a folder to make it a project. A background worker checks added, changed, deleted, and renamed files while respecting .gitignore and skipping large dependency folders. Powered by isomorphic-git, with no Git installation required.',
    ...
    '问题全流程管理': 'Manage issues in one place',
    '创建问题、回复评论、关闭与重新打开；问题列表按项目分组，从全局进入默认收起、从项目详情进入自动展开对应项目。': 'Create issues, reply, close, and reopen them. Issues are grouped by project so it is easy to focus on one conversation at a time.',
    ...
    '在“发现 / 搜索”里浏览近期活跃、受关注的公开项目。EasyHub 结合 Star 数、近期活动和更新时间排列结果，帮你从更多作品里找到感兴趣的项目。': 'Browse active, well-liked public projects in Discover. EasyHub ranks them using stars, recent activity, and update time to help you find your next favorite project.',
    ...
    'Windows 1.0.2 安装版与便携版': 'EasyHub 1.0.2 for Windows: installer and portable app',
    ...
    '现有 72 项单元测试与界面点击测试，交付前可完整检查：': 'Run the available unit and UI checks before building:',
    ...
    '© 2026 EasyHub · v1.0.2 · 本页对比信息基于各项目 2026 年 9 月公开资料整理': '© 2026 EasyHub · v1.0.2 · Comparison based on public project information from September 2026',
